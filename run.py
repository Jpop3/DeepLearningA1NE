...
CEL = nn.fit(input_training, labels_training, input_val, labels_val, learning_rate=0.0005, epochs=10, batch_size=2) #can change parameters
...
for index, array in enumerate(output):
    if np.argmax(array) == np.argmax(one_hot_labels[index]): #the largest val in the vector indicates what class its predicting.
        correct_count += 1
print(correct_count/50000) #accuracy
...
for index, array in enumerate(output):
    if np.argmax(array) == np.argmax(test_one_hot_labels[index]):
        correct_count += 1
print(correct_count/10000) #accuracy
